...
# command to run:  python3 /home/pi/L298N_flexbot01/sensor-C-distance.py
...
def Measure(): 
    ...
    flexbot01_gpio.setlow_ultra(pinTrigger, 100000, debug) # 100 ms settling period
    # send a 10us HIGH to trigger the sensor's ranging program (which sends 8 ultrasound bursts at 40 kHz)
    flexbot01_gpio.send_ultra(pinTrigger, 10, debug)
    InitTime = time.time()
    StartTime = time.time() 
    StopTime = StartTime
    tooshort = "no"
    ...
    while flexbot01_gpio.read_pin(pinEcho)==0: 
        StartTime = time.time() 
        StopTime = StartTime
    ...
    while flexbot01_gpio.read_pin(pinEcho)==1: 
        StopTime = time.time() 
...
flexbot01_gpio.setIO_GPIO(pinTrigger, 1)   # Trigger 
flexbot01_gpio.setIO_GPIO(pinEcho, 0)      # Echo
